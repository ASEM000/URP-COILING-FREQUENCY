...
@jit(nopython=True,parallel=True)   
def jet_center(array):
    '''
    input  : 3D denoised video numpy array ( frame, row,col)
    output : 2D jet center - time(frame)   (row,jet centers over frames)
    '''
    frames,rows,cols,channel = array.shape
    result = np.zeros((rows,frames))
    for ri in prange(rows) :
        for fi in prange(frames):
            locs   =  np.where(array[fi,ri,:,0] == 255 )[0] #get jet center a frame fi and row ri
            if len(locs) > 0: center = 0.5*(locs[-1] + locs[0]) ; result[ri,fi] = center #assign the value to row-> frame
            else : result[ri,fi] = -1   #no center found
    return result   


def analyze_jet_center(array , threshold_value , tolerance):
    '''
